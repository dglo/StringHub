...
public interface IDOMApp 
{
     ...
     * @param mask set the LED bitmask.  This directs which of the 12 LEDs 
     *  will fire.
     * @param rate set the LED flashing rate.  
     *  Note that the actual rate may be rounded by the HAL.
     */
    void beginFlasherRun(short brightness, short width, short delay,
        short mask, short rate) throws MessageException;

    /**
     * Similar to beginFlasherRun message but doesn't need to have a 
     * powered-off flasher initial state.  
     * @param brightness
     * @param width
     * @param delay
     * @param mask
     ...
     */
    void collectPedestals(int nAtwd0, int nAtwd1, int nFadc, Integer... 
        avgPedestals) throws MessageException;
    
    void setChargeStampType(boolean fADC, boolean autoRange, byte chan) 
        throws MessageException;
    ...
    /**
     * Disable readout of every 8192nd waveform even if it doesn't meet 
     * HLC requirement.
     * @throws MessageException
     */
    ...
    void histoChargeStamp(int interval, short prescale) 
        throws MessageException;
     ...
     * @return 2-element array [0] = ADC value (readback) [1] = 
     *  DAC value (set)
     * @throws MessageException
     */
