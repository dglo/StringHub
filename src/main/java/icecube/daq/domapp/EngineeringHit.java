...
public class EngineeringHit 
{
    ...
    private static final Logger logger = 
        Logger.getLogger(EngineeringHit.class);
    ...
    public EngineeringHit(ByteBuffer buf) 
    {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Decode recl = " + len + " (" + 
                engRecFmt.fadcSamples() + ", " + engRecFmt.atwdSamples(0) + 
                ", " + engRecFmt.atwdSamples(1) + ", " + 
        ...
        for (int i = 0; i < engRecFmt.fadcSamples(); i++) {
            fadc[i] = buf.getShort();
        }
    ...
    { 
        return Trigger.values()[triggerFlags & 3]; 
    }

    public short[] getAtwd(int ch) 
    {
        return atwd[ch]; 
    }
