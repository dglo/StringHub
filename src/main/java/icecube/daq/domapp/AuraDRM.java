...
public class AuraDRM extends IcebootInterface
{
     ...
     */
    public boolean writePowerBits(int pwr) 
        throws IOException, InterruptedException
    {
        ...
        int powerTry = 0;
        while (readVirtualAddress(4) != pwr && powerTry++ < MAX_POWER_TRY) {
            Thread.sleep(100);
        }

        logger.warn("Trying to set power to " + pwr);
        if (powerTry <= MAX_POWER_TRY) {
            return true;
        }
    ...
    /**
     * This command queues up the radio DACs for a write operation.  
     * To actually write the DACs follow up with writeRadioDACs() 
     * @param ant
     * @param band
     * @param val
     * @throws IOException
    ...
    /**
     * This command actually effects the write for the DACs set by the 
     * setRadioDAC call which must have been previously called one 
     * or more times.
     * 
     */
    ...
    private ByteBuffer readTRACRData(String cmd, int nEvents) 
        throws IOException
    {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Got ByteBuffer(" + p + " bytes) from DOM.");
        }
