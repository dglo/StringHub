...
public class DOMApp implements IDOMApp
{
            ...
            short rate) throws MessageException
    {
        ...
        buf.putShort(brightness).putShort(width).putShort(delay).
            putShort(mask).putShort(rate).flip();
        sendMessage(MessageType.CHANGE_FB_SETTINGS, buf);
    }
    
    public void beginFlasherRun(short brightness, short width, 
        short delay, short mask, short rate) throws MessageException
    {
        ByteBuffer buf = ByteBuffer.allocate(10);
        buf.putShort(brightness).putShort(width).putShort(delay).
            putShort(mask).putShort(rate).flip();
    ...
    public void collectPedestals(int nAtwd0, int nAtwd1, 
        int nFadc, Integer... avgPedestals) throws MessageException
    {
    ...
    public MuxState getMux() throws MessageException
    {
        ...
        case -1: 
            return MuxState.OFF;
        case 0: 
            return MuxState.OSC_OUTPUT;
        case 1:
            return MuxState.SQUARE_40MHZ;
        case 2: 
            return MuxState.LED_CURRENT;
        case 3: 
            return MuxState.FB_CURRENT;
        case 4: 
            return MuxState.UPPER_LC;
        case 5: 
            return MuxState.LOWER_LC;
        case 6: 
            return MuxState.COMM_ADC_INPUT;
        case 7: 
            return MuxState.FE_PULSER;
        default: 
            throw new MessageException(MessageType.GET_MUX_CH,
                new IllegalArgumentException(muxCh + 
                    " is not a valid MUXer channel"));
        }
    }
    /*
     * (non-Javadoc)
     ...
     * These histograms will be emitted in the monitoring records.  The 
     * histogram length is 127 bytes so the prescale argument is used to 
     * set the range.
     * @param interval interval in clock ticks (> 40M) or seconds (< 40M)
     * @param prescale divisor for the chargestamp.
     * @throws MessageException
     */
    public void histoChargeStamp(int interval, short prescale) 
        throws MessageException
    {
     ...
     */
    protected ByteBuffer sendMessage(MessageType type, ByteBuffer in) 
        throws MessageException
    {
        ByteBuffer buf = (ByteBuffer) msgBuffer.clear();
        ...
        try {
            ...
            while (msgBufferOut.position() < 8 || msgBufferOut.position() < 
                msgBufferOut.getShort(2) + 8) 
            {
                Thread.yield();
                ByteBuffer out = devIO.recv();
            ...
            if (!(type.equals(msgBufferOut.get(0), 
                msgBufferOut.get(1)) && status == 1)) 
            {
                throw new MessageException(type, msgBufferOut.get(0), 
                    msgBufferOut.get(1), status);
            }
    ...
    public void setChargeStampType(boolean fADC, boolean autoRange, byte chan) 
        throws MessageException
    {
     ...
     * (non-Javadoc)
     *
     * @see ic3.daq.domapp.IDOMApp#setEngineeringFormat
     * (ic3.daq.domapp.EngineeringRecordFormat)
     */
        ...
        throws MessageException
    {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Setting engineering format bytes to (" + 
                enc[0] + ", " + enc[1] + ", " + enc[2] + ").");
        }
    ...
    public LBMDepth getLBMDepth() throws MessageException
    {
        ...
        switch (length) {
        case 0x000100: 
            return LBMDepth.LBM_256;
        case 0x000200: 
            return LBMDepth.LBM_512;
        case 0x000400:
            return LBMDepth.LBM_1K;
        case 0x000800: 
            return LBMDepth.LBM_2K;
        case 0x001000: 
            return LBMDepth.LBM_4K;
        case 0x002000: 
            return LBMDepth.LBM_8K;
        case 0x004000: 
            return LBMDepth.LBM_16K;
        case 0x008000: 
            return LBMDepth.LBM_32K;
        case 0x010000: 
            return LBMDepth.LBM_64K;
        case 0x020000: 
            return LBMDepth.LBM_128K;
        case 0x040000: 
            return LBMDepth.LBM_256K;
        case 0x080000: 
            return LBMDepth.LBM_512K;
        case 0x100000: 
            return LBMDepth.LBM_1M;
        case 0x200000: 
            return LBMDepth.LBM_2M;
        case 0x400000: 
            return LBMDepth.LBM_4M;
        case 0x800000: 
            return LBMDepth.LBM_8M;
        default:
            throw new MessageException(MessageType.GET_LBM_DEPTH, 
     ...
     * (non-Javadoc)
     *
     * @see ic3.daq.domapp.IDOMApp#setLCMode
     * (ic3.daq.domapp.LocalCoincidenceConfiguration.RxMode)
     */
     ...
     * (non-Javadoc)
     *
     * @see ic3.daq.domapp.IDOMApp#setLCTx
     * (ic3.daq.domapp.LocalCoincidenceConfiguration.TxMode)
     */
     ...
     * (non-Javadoc)
     *
     * @see ic3.daq.domapp.IDOMApp#setLCType
     * (ic3.daq.domapp.LocalCoincidenceConfiguration.Type)
     */
    ...
    public boolean isRunningDOMApp()
        throws IOException, InterruptedException
    {
        ...
        if (ack.get(4) != (byte) 0x45) {
            return true;
        }
        // finish up reading iceboot response
        while (ack.position() < 34) {
            ack.put(devIO.recv());
        }
        return false;
    }
        ...
        throws IOException, InterruptedException
    {
        ...
        // command decoder
        String status = talkToIceboot(
            "s\" domapp.sbi.gz\" find if gunzip fpga endif . set-comm-params");
        if (logger.isDebugEnabled()) {
    ...
    private String talkToIceboot(String cmd, String expect) throws IOException
    {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Echoback from iceboot received - expecting ... " +
                expect);
        }
    ...
    public FastMoniRateType getFastMoniRateType() throws MessageException
    {
        ...
        byte type = buf.get();
        if (type == 0) {
            return FastMoniRateType.F_MONI_RATE_HLC;
        }
        return FastMoniRateType.F_MONI_RATE_SLC;
    }

    public void setFastMoniRateType(FastMoniRateType type) 
        throws MessageException
    {
        ByteBuffer buf = ByteBuffer.allocate(1);
