...
public final class DeltaCompressedHit
{
    ...
    private static final Logger logger = 
        Logger.getLogger(DeltaCompressedHit.class);
    ...
    public static DeltaCompressedHit decodeBuffer(ByteBuffer buf, int clockMSB)
    {
        ...
        for (TriggerBit trigBit : TriggerBit.values()) {
            if ((word1 & (1 << (18 + trigBit.ordinal()))) != 0) {
                hit.triggerMask.add(trigBit);
            }
        }
        ...
        if (hit.fadcAvailable) {
            hit.fADC = codec.decode(256);
        }
        if (hit.atwdAvailable) {
            for (int i = 0; i < natwd + 1; i++) {
                hit.atwd[i] = codec.decode(128);
            }
        }
        codec.alignBuffer();
        return hit;
    ...
    public EnumSet<TriggerBit> triggers() 
    { 
        return triggerMask; 
    }
    public boolean hasFADC() 
    { 
        return fadcAvailable; 
    }
    public boolean hasATWD() 
    { 
        return atwdAvailable; 
    }
