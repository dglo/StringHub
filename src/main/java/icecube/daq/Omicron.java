...
public class Omicron 
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        if (args.length - index < 2) {
            System.err.println(
                "usage : java ic3.daq.Omicron [ opts ] <output-file> <xml-config>");
            System.exit(1);
        }
        ...
        for (DOMChannelInfo chInfo : activeDOMs) {
            if (xmlConfig.getDOMConfig(chInfo.mbid) != null) {
                nDOM++;
            }
        }

        FileOutputStream fOutHits = 
            new FileOutputStream(outputBaseName + ".hits");
        FileOutputStream fOutMoni = 
            new FileOutputStream(outputBaseName + ".moni");
        FileOutputStream fOutTcal = 
            new FileOutputStream(outputBaseName + ".tcal");
        FileOutputStream fOutScal = 
            new FileOutputStream(outputBaseName + ".scal");

        BufferConsumerChannel hitsChan = 
            new BufferConsumerChannel(fOutHits.getChannel());
        BufferConsumerChannel moniChan =
            new BufferConsumerChannel(fOutMoni.getChannel());
        BufferConsumerChannel tcalChan = 
            new BufferConsumerChannel(fOutTcal.getChannel());
        BufferConsumerChannel scalChan = 
            new BufferConsumerChannel(fOutScal.getChannel());
        
        MultiChannelMergeSort hitsSort = 
            new MultiChannelMergeSort(nDOM, hitsChan, "hits");
        MultiChannelMergeSort moniSort = 
            new MultiChannelMergeSort(nDOM, moniChan, "moni");
        MultiChannelMergeSort tcalSort = 
            new MultiChannelMergeSort(nDOM, tcalChan, "tcal");
        MultiChannelMergeSort scalSort = 
            new MultiChannelMergeSort(nDOM, scalChan, "supernova");
        ...
        for (DOMChannelInfo chInfo : activeDOMs) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("DataCollector thread on (" + 
                    chInfo.card + "" + chInfo.pair + "" + chInfo.dom +
                        ") started.");
            }
        }
        ...
        for (DataCollector dc : collectors) {
            if (!dc.isAlive()) {
                logger.warn("Collector " + dc.getName() + 
                    " died before config: schedule for removal.");
                reaper.add(dc);
            } else {
                dc.signalConfigure();
            }
        ...
        for (DataCollector dc : collectors) {
            if (!dc.isAlive()) {
                logger.warn("Collector " + dc.getName() + 
                    " died during config: schedule for removal.");
                reaper.add(dc);
            } else {
                while (!dc.getRunLevel().equals(RunLevel.CONFIGURED) && 
                    ...
                    {
                        if (logger.isDebugEnabled()) {
                            logger.debug("Waiting of DC " + dc.getName() + 
                                " to configure.");
                    }
                    Thread.sleep(500);
                }
                if (!dc.getRunLevel().equals(RunLevel.CONFIGURED)) {
                    logger.warn("Collector " + dc.getName() + 
                        " stuck configuring: coup de grace.");
                    dc.interrupt();
                    reaper.add(dc);
                }
            }
        ...
        for (DataCollector dc : collectors) {
            while (dc.isAlive() && !dc.getRunLevel().
                equals(RunLevel.CONFIGURED)) 
            {
                Thread.sleep(100);
            }
            dc.signalShutdown();
