...
public abstract class AbstractRAPCal implements RAPCal
{
    class Isochron
    {
        ...
        private UTC[] setupVierling(TimeCalib tcal) throws RAPCalException
        {
            UTC[] t = new UTC[4];
            t[0] = tcal.getDorTx();
            t[1] = UTC.add(tcal.getDomRx(), getFineTimeCorrection(
                tcal.getDomWaveform()));
            t[2] = tcal.getDomTx();
            t[3] = UTC.add(tcal.getDorRx(), getFineTimeCorrection(
                tcal.getDorWaveform()));
            return t;
        }
        ...
        private void proc()
        {
            long dor_dt = UTC.add(t1[0], t1[3]).subtractAsUTC(
                UTC.add(t0[0], t0[3])).in_0_1ns() / 2L;
    ...
    {
        this(Double.parseDouble(System.getProperty(
            "icecube.daq.rapcal.AbstractRAPCal.expWeight", "0.1")));
    }
    ...
    public AbstractRAPCal(double w)
    {
        ...
        BASELINE_SAMPLES = 20;
        MAX_HISTORY = Integer.getInteger(
            "icecube.daq.rapcal.AbstractRAPCal.history", 10);
    }
